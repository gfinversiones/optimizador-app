...
.title { font-size: 26px; font-weight: 600; letter-spacing: 0.5px; }
...
.metric-label { font-size: 11px; color: #7f8c99; letter-spacing: 1px; }
.metric-value { font-size: 20px; font-weight: 600; color: #ffffff; }
...
.port-title { font-size: 13px; font-weight: 600; margin-bottom: 8px; }

section[data-testid="stSidebar"] { background-color: #0f141a; }
</style>
...
with st.sidebar:
    ...
    tickers = [t.strip().upper() for t in st.text_input("Tickers", "AAPL,MSFT,GOOGL,AMZN").split(",")]
...
draw_portfolio(col1, "MAX_SHARPE", r_s, v_s, res_s.x)
draw_portfolio(col2, "MIN_VOL", r_m, v_m, res_m.x)
...
# ✅ FIX CLAVE
st.plotly_chart(copy.deepcopy(fig), use_container_width=True, key="frontier_chart")