...
# Crecimiento base de tránsito:
# Y2→Y3: +5% (FLUJO fila 16 col D); Y3+ en adelante: +3% constante
TRAFICO_CRECIMIENTO_BASE = 0.03   # tasa de largo plazo (Y3+)
TRAFICO_CRECIMIENTO_Y2   = 0.05   # salto Y2→Y3
...
# ── Amortización impositiva (AMORTIZACIONES fila 98 – escenario 20/5 años) ──
AMORT_IMP_BASE = np.array([
    ...
    12_734_189_999.999,   # Y12  (nota: Y12 obras termina, repav sigue)
    6_945_921_818.18,     # Y13
    4_051_787_727.27,     # Y14
    5_788_268_181.82,     # Y15
    8_682_402_272.73,     # Y16
    12_300_069_886.36,    # Y17
    21_947_183_522.73,    # Y18
    33_523_719_886.36,    # Y19
    59_570_926_704.55,    # Y20
], dtype=float)
...
def run_model(
    ...
    # ── Tráfico ────────────────────────────────────────────────
    # Y1: 0 (puesta en valor)
    # Y2: UTEQ_ARRANQUE (primer año de cobro)
    # Y3: Y2 × (1 + 5%) – salto año 2→3 según xlsx fila 16
    # Y4+: crece a tasa_eff constante
    uteq = np.zeros(YEARS)
    uteq[0] = 0.0
    uteq[1] = UTEQ_ARRANQUE
    tasa_eff = TRAFICO_CRECIMIENTO_BASE + delta_trafico
    tasa_eff = max(tasa_eff, -0.50)
    uteq[2] = uteq[1] * (1 + TRAFICO_CRECIMIENTO_Y2)   # +5% fijo Y2→Y3
    for y in range(3, YEARS):
    ...
    uteq_base_y = UTEQ_ARRANQUE
    for y in range(1, YEARS):
        if y == 1:
            ub = UTEQ_ARRANQUE
        elif y == 2:
            ub = UTEQ_ARRANQUE * (1 + TRAFICO_CRECIMIENTO_Y2)
        else:
            ub = UTEQ_ARRANQUE * (1 + TRAFICO_CRECIMIENTO_Y2) * ((1 + TRAFICO_CRECIMIENTO_BASE) ** (y - 2))
...
with st.sidebar:
    ...
    st.caption(
        ...
        "**Año 3** → +5% sobre Año 2 (xlsx).  \n"
        "**Año 4+** → crece a tasa base 3% + Δ abajo."
    )

    delta_trafico_pp = st.slider(
        "Δ tasa crecimiento anual Año 4+ (±pp sobre base 3%)",
