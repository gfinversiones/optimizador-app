...
with tab_stress:
    ...
    st.dataframe(
        df_hip.style.applymap(color_negativo)
                    .format("{:.2f}%"),
        use_container_width=True,
        height=210,
    )
    ...
    st.dataframe(
        df_hist.style.applymap(color_hist)
    ...
    colores_c = ["#1a56db", "#e53935", "#2e7d32", "#7209B7"]
    ...
    if spy_p_st is not None:
        ...
        if len(spy_c) > 1:
            ...
            fig_crisis.add_trace(go.Scatter(
                ...
                name="SPY", line=dict(color="#b8860b", width=1.5, dash="dash"),
            ))
    ...
    fig_crisis.update_layout(
        ...
        plot_bgcolor="rgba(0,0,0,0)",
        xaxis=dict(title="Fecha",    gridcolor="rgba(128,128,128,0.15)"),
        yaxis=dict(title="Retorno Acumulado (%)", gridcolor="rgba(128,128,128,0.15)"),
